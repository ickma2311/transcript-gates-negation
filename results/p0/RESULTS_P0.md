# RESULTS P0 — 2026-09-23 (EXPLORATORY; Colab T4; code job_bundle_p0_frozen.py; PREREG_P0 sha256 3637272e…)

## Verdict: NOT GO on both models (H2 holds; H1 and H3 do not; the confirm gate is not "dead", so STOP is not triggered) → INCONCLUSIVE per the pre-registration, enter lite mode

## Strict scoring (the pre-registered scorer; over-penalises free-text string arguments — see the audit)
...
## Lenient re-scoring (DEVIATION, post hoc, offline: function name + all boolean/integer/enum arguments must match; free-text strings ignored; `rescore.py`)
```
...
## Pre-registered contrasts under strict scoring
```
Pre-registered contrasts:
...
## Scorer audit (raw outputs scored WRONG on clean/none)
```
...
## The 7B model's 4 CONFIRM replies on neg_drop (each asks about exactly the flipped argument)
```
