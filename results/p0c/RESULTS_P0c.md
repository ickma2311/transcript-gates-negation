# RESULTS P0c — 2026-09-24 (EXPLORATORY; Colab T4 1005 s; code job_bundle_p0c.py; PREREG_P0c sha256 502fdfaf…; the first run was lost to a client no-output timeout, the second completed after switching to streamed output; frozen file hashes unchanged)

## Verdict: CLOSE on both models → route (b) closed per the pre-registration
- prompted (confirm_optbool): blocked(neg_drop) 3B 0.0% / 7B 2.4%; pooled clean CF 0% / 0% — the rule **was not executed**: the models still set the optional boolean to true and call directly (OK 95.2% / 100% on clean_pos).
- wrapper (deterministic post-filter): blocked 97.6% / 97.6% (by construction), but clean_pos CF 97.6% / 100%, pooled over 84 cards 51.2% — the cost is that 41/42 (3B) and 42/42 (7B) of the legitimate "turn on X" requests need confirmation.
- The pre-registered expectation (prompted ≈ wrapper, cost ≈ 50%) was **half wrong**: the prompted version's cost is 0%, not 50%, because it never fires at all; the LLM did not treat the "argument-shape rule" as a policy to follow.

## Lenient scoring (DEVIATION as in P0)
```
