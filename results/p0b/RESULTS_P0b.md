# RESULTS P0b — 2026-09-23 (EXPLORATORY; Colab T4 1169 s; code job_bundle_p0b.py; PREREG_P0b including the action-level revision note)

## Verdict: per PREREG_P0b, the original H1 **is not a sentence-end artefact; it is absent** → the direction is downgraded to a negative result
- Confirmation rate on other_1w_mid (in-sentence insertion) @confirm: 3B 0.0%, 7B 2.4% (lower than the sentence-end 4.8% / 9.5%)
- confirm_generic (names no error type): neg_drop 0.0 / 2.4%, other_intent 28.6 / 16.7%, clean 0 / 0% — the same pattern as the leading prompt
- Action-level DiD (WA reduction under confirm on other_1w_mid − on neg_drop): 3B −4.8 [−11.9, 0.0]; 7B −9.5 [−19.0, −2.4] (if there is an interaction, its sign is opposite to the original hypothesis: 7B's confirmations rescued negation slightly more). Note: 3B's other_1w (sentence-end) DiD is +2.4 [0.0, 7.1], the only positive cell, still far below 25pp.
...
## Contrasts under strict scoring
```
...
## Full tables under strict scoring
```
